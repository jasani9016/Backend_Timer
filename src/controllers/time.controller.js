...
const checkTimerStart = catchAsync(async (req, res) => {
  const time = await Time.findOne({ user: req.user.id, endTime: null }).sort({
    createdAt: -1,
  });
...
const todayTotalCount = catchAsync(async (req, res) => {
  ...
  const formattedTotal = `${padZero(totalHours)}:${padZero(
    totalMinutes
  )}:${padZero(totalSeconds)}`;

  return res
    .status(httpStatus.OK)
    .send({ total: formattedTotal, totalElapsedSeconds: total });
});
